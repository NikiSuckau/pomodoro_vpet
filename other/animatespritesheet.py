...
BG_WIDTH = 32  # Background width (enough space for movement)
BG_HEIGHT = 24
SPRITE_Y = 0  # y position is fixed (top-aligned)
...
STEP_LENGTH = 12  # pixels covered per walk cycle (left+right)
...
walk_right_1 = frames[0].transpose(Transpose.FLIP_LEFT_RIGHT)
walk_right_2 = frames[1].transpose(Transpose.FLIP_LEFT_RIGHT)
...
happy_frames = [frames[3], frames[2]]
happy_steps = 7
...
# 3. Attack (center)
attack_frames = [frames[4], frames[7]]
attack_steps = 5
...
# 4. sleep (center)
sleep_frames = [frames[6], frames[6]]
sleep_steps = 6
...
for i in range(walk_total_frames_out):
    anim_idx = i % 2
    walk_frame = frames[0] if anim_idx == 0 else frames[1]  # walk left
...
MAGNIFICATION = 4  # Set to 1, 2, 4, or 8
if MAGNIFICATION > 1:
    scaled_frames = [
        f.resize(
...
scaled_frames[0].save(
    ...
    duration=600,  # 300 ms per frame (about 3 fps, adjust as needed)
    loop=0,
    disposal=2,
    transparency=0,
)

os.system('xdg-open "output_animation.gif"')
